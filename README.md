...
| **多工具调用兼容** | AI 单次推理调用多个工具或多轮工具调用时，按执行顺序临时组织文本与工具结果；持久保存前清除内部工具协议块 |
...
```json
{
  ...
  "proactive_probability_scale": 0.5,
...
- **多工具调用兼容** — AI 在单次推理中调用多个工具或发生多轮工具调用时，按实际执行顺序临时组织 AI 中间文本与工具结果；兼容 ToolCall 对象和 dict 两种格式，持久保存前清除内部工具协议块
