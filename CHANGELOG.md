...
**修复与改进**:
- 新增工具调用控制文本发送前防护，拦截 `<tool_call>`、`<function=...>` 等标记进入群聊
...
- 群聊处理期间临时包装 `event.send`，拦截平台层 `tool_call`、`tool_call_result`、`<tool_calls>` 和“调用工具”状态消息，避免工具循环状态被直接发到群聊
...
- 扩展工具外显防护测试，覆盖平台工具状态识别、群聊 `event.send` 过滤器安装、旧 `<tool_call>` 历史协议标记清理和缓存转正清理入口
...
- 工具返回值仅发送生成图片，避免把 API Key、接口地址、原始响应和临时文件信息写入群聊
...
- 将工具调用前的无关视觉能力拒绝文本改为 Step Image 进度提示，并要求模型调用图片工具时不额外输出普通聊天正文
- Step Image 图片结果改为工具内部发送并向 Agent 返回空结果，绕开响应阶段的图片结果复用，进一步避免同一图片重复发送
