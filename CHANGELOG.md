...
- Provider 列表获取失败继续映射为固定安全查询错误；枚举时逐个跳过元数据异常或无有效 ID 的 Provider 成员，并继续查找后续目标
- 调用前检测 `generate_image()` 是否支持显式 `timeout` 或 `**kwargs`；支持时传入与外层等待相同的单次超时，旧签名或签名读取异常时保留原参数且只调用一次。生产 Codex OAuth Provider 已支持单次超时参数
- 内部 LLM 工具名保持为 `gcp_step_image_generate` 与 `gcp_step_image_edit`
- 进度文本随图片后端变化，图片结果只发送一次，主模型按当前群人格输出自然语言收尾
- 主流程只在当前处理期间临时构造交错工具记录
- `ContextManager.save_bot_message()` 保存前清除工具协议块
- 自定义历史与官方会话写入清理后的文本
- 后续格式化为模型上下文时再次过滤旧数据或外部数据，群聊输出不包含工具协议块
...
**修复与改进**:
- LivingMemory 记忆注入会读取 `sender_id`、`user_id`、`group_id`、`participants` 等结构化字段，并将成员格式化为 `昵称(ID:QQ)` 或 `昵称(仅昵称来源，身份未确认)`
- 背景信息注入新增成员身份识别规则，要求涉及具体成员时以用户ID为准，昵称只作展示
- 平台 LTM 图片描述匹配增加 `sender_id` 参与判定；当 LTM 记录带 ID 时优先按 ID 区分同昵称成员，旧的仅昵称记录仍只作为当前图片内容补充

**验证**:
- 新增 `tests/test_identity_hardening.py`，覆盖 LivingMemory 身份可信度标注、记忆注入规则、同昵称图片记录 ID 匹配和仅昵称 LTM 记录兼容
...
**修复与改进**:
- 新增工具调用控制文本候选清理，避免 `<tool_call>`、`<function=...>` 等标记被选为群聊自然回复
- 当工具调用标记混入自然回复时，发送前清理控制标记并保留正常文本
- 记录“提示词可见工具”和“执行器可用工具”的差异，辅助排查跨插件 LLM Tool 注册问题
- 历史上下文注入前剥离 `[工具调用记录]`、`[工具调用结束]`、`SYSTEM NOTICE` 和工具执行占位文本，避免旧工具记录被模型复述
- AI 回复保存到自定义历史与官方会话前执行同一清理逻辑；纯工具审计文本不再作为 AI 回复写入历史
- 平台层工具状态显示继续交由 AstrBot `show_tool_use_status` 与 `show_tool_call_result` 控制，本插件只清理自身回复候选、历史候选和保存候选
- 历史上下文清理扩展到 `<tool_call>`、`<function=...>` 等协议标记，降低旧历史或外部写入记录再次进入模型上下文的概率
- 缓存消息转正到官方会话前同步清理工具审计文本；清理后为空的缓存项跳过写入，转正计数按实际新增条数计算

**验证**:
- 新增 `tests/test_tool_call_leakage_guard.py`，覆盖纯工具调用标记拦截、混合文本清理和工具表差异日志接入点
- 扩展 `tests/test_multimodal_history_content.py`，覆盖 Step Image 工具记录与平台工具状态文本的历史注入清理
- 扩展工具外显防护测试，覆盖禁止群聊流程包装 `event.send`、旧 `<tool_call>` 历史协议标记清理和缓存转正清理入口
...
**新增**:
- 增加 StepFun `step-image-edit-2` 文生图与修图工具，启用本插件的群聊可在正式回复阶段通过工具调用返回图片
- 文生图支持 `1024x1024`、`768x1360`、`896x1184`、`1360x768`、`1184x896` 五种尺寸
- 修图会读取当前消息中的第一张图片，要求图片与编辑指令位于同一条群消息
- 复用已配置的 Provider 读取 API Key、`api_base`、超时和代理，也支持在插件配置中覆盖图片接口参数
- 工具发送图片后返回安全结果摘要，交给主模型按当前人格生成自然语言收尾，避免把 API Key、接口地址、原始响应和临时文件信息写入群聊
- 文生图默认尺寸改为 `768x1360`，接近 16:9 / 1080P 横屏，并支持 `1080p`、`16:9`、`1920x1080` 等常见比例和分辨率别名
- 工具说明强化为由正式回复模型先整理图像提示词，再调用 StepFun 图片接口，避免把群聊原文和内部判断文本原样交给图片模型

**修复与改进**:
- Step Image 工具调用后先发送人格化进度提示，再发送图片结果
- 清理工具直发图片后遗留的响应结果，避免同一张图片在群聊里发送两次
- 将工具调用前的无关视觉能力拒绝文本改为 Step Image 进度提示；工具返回结果后由主模型根据成功或失败状态生成群聊自然语言回复
- Step Image 图片结果改为工具内部发送并向 Agent 返回安全短摘要，绕开响应阶段的图片结果复用，进一步避免同一图片重复发送
- Step Image 群聊守卫增加 `GroupMessage` 来源、`session_id` 与消息对象群号回退，并统一群号字符串匹配，避免启用群内部分调用被误判为未授权
- 生图/修图请求会过滤历史、上下文和外部记忆中关于“视觉塔”“画不了”等过期图片能力记录，同时追加当前 Step Image 能力规则

**验证**:
- 新增 `tests/test_step_image_service.py`，覆盖文生图、修图、Provider 自动发现、参数校验和错误脱敏
- 新增 `tests/test_step_image_tool_integration.py`，覆盖工具注册、群聊守卫、进度提示、重复图片处理、旧能力文本过滤和配置面板字段
...
**修复与改进**:
- 将主模型最终回复判断从正式回复生成提示词中拆分出来，保留“读空气 AI 粗筛 + 最终回复模型二次判断”的两阶段判断，同时避免判断指令和控制标记进入正式生成上下文
- 最终回复判断改用插件内部控制标记，旧版 `[[NO_REPLY]]` 仅作为兼容清理项，减少控制文本被发送到群聊的可能
- 群聊回复、主动对话和私聊回复提示词改为“直接生成要发送内容”的表达，强调不要输出是否回复、内部取舍、工具来源和系统提示等元信息
- Web 管理面板中的默认提示词预览同步更新，避免界面展示内容与运行时默认提示词不一致

**验证**:
- 新增 `tests/test_reply_leakage_guard.py`，覆盖最终判断拆分、内部控制标记、群聊/私聊/主动对话提示词和 Web 预览一致性
