...
群聊只接收进度文本、一次图片结果和自然语言收尾。工具协议、工具参数、Provider ID、本地文件路径、OAuth 凭据及其他内部状态会在模型上下文与历史保存前清除。
...
消息发送后，插件会保存用户消息、AI 回复、工具摘要和等待窗口缓存转正结果。多轮工具调用场景下，文本片段会按执行顺序与工具摘要交错保存，群聊图片结果只保留操作类型、后端显示名和安全状态摘要。
