...
正式回复阶段通过 AstrBot 原生 LLM 请求流程执行，当前会话可见的搜索、MCP、知识库、沙箱、Pixiv 类工具以及本插件 StepImage 工具会随请求进入工具循环。判断阶段不调用工具，减少外部副作用。

StepImage 工具命中时，插件会先发送人格化进度提示，工具负责发送图片结果，随后主模型根据工具返回的安全摘要生成自然语言收尾。工具参数、调用详情和 provider 信息不会发送到群聊。
...
消息发送后，插件会保存用户消息、AI 回复、工具摘要和等待窗口缓存转正结果。多轮工具调用场景下，文本片段会按执行顺序与工具摘要交错保存，StepImage 图片结果只保留安全摘要。
