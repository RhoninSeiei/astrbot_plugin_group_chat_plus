...
Phase 9 · 正式回复阶段工具循环
    ↓
Phase 10 · 结果装饰（泄漏防护/短回复限制/错字/延迟）
    ↓
Phase 11 · 发送后保存（用户消息/AI 回复/工具摘要/缓存转正）
```
...
读空气 AI 通过后，主模型会再进行一次最终判断。该阶段复用当前会话的人格、模型与上下文设置，只用于决定本轮是否值得进入正式回复，避免粗筛阶段误判导致频繁插话。

最终判断的群聊文本最多保留 6000 字符，优先保留当前消息和等待窗口，再使用剩余预算从最新历史消息向前回填，并恢复时间顺序。不再限制为最近 6 条；遇到放不下的完整消息时停止回填，不跳过较新的消息去选择更早的消息。关闭发送者和时间戳后，如果无法识别消息边界，则整段保留或舍弃历史。当前消息与等待窗口自身超出预算时保留尾部。该预算不包含独立的人格与判断提示词。

正式回复仍使用现有上下文配置和附加信息，不受此精简预算影响。

OAuth 接口适配：读空气、通用判断和最终判断请求显式使用 `tool_choice="none"`，避免判断阶段调用工具。判断、正式回复及人格错误提示请求均使用 `oauth_web_search="disabled"` 和 `retry_rate_limits=False`；正式回复同时使用 `fallback_on_rate_limit=False`，最终判断自行阻止 HTTP 429 异常或结构化错误响应触发备用 Provider。其他错误保留核心原有重试和备用策略。

插件不主动启用 OAuth 托管搜索、音频转写或实时语音，不自动退回 API Key。正式回复仍允许普通函数工具。请求策略不修改共享 Provider 配置；正式回复的后续工具轮次、流式请求和备用路径由 AstrBot 核心传递这些策略。本适配依赖包含请求级策略接口的 AstrBot 自用核心提交 `33794cadd7b368cf6a396206f37fa4f13cec6846` 或等效实现，不能仅凭版本号 4.27.5 判定兼容。

主模型最终判断通过后进入正式回复阶段；未通过时，本轮消息仍会按上下文规则保存，供后续对话参考。

---

## Phase 9 · 正式回复阶段工具循环

正式回复阶段通过 AstrBot 原生 LLM 请求流程执行，当前会话可见的搜索、MCP、知识库、沙箱、Pixiv 类工具以及本插件群聊图片工具会随请求进入工具循环。判断阶段不调用工具，减少外部副作用。
...
群聊只接收进度文本、一次图片结果和自然语言收尾。主流程只在当前处理期间临时构造交错工具记录，群聊输出不包含工具协议块。
...
`ContextManager.save_bot_message()` 保存前清除工具协议块，自定义历史与官方会话写入清理后的文本。后续格式化为模型上下文时再次过滤旧数据或外部数据，避免旧记录或外部写入的协议内容进入模型上下文。

图片安全摘要只包含操作类型、成功或失败状态和安全消息，不包含后端显示名。Provider ID、凭据、API 地址、原始响应和文件路径不会进入安全摘要或群聊文本。群聊输出不包含工具协议块。
