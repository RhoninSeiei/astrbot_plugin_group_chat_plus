...
群聊只接收进度文本、一次图片结果和自然语言收尾。内部工具调用记录属于持久历史审计内容，不作为群聊回复发送；Provider ID、凭据、API 地址、原始响应和文件路径不会写入图片工具的安全摘要或群聊文本。
...
消息发送后，插件会保存用户消息、AI 回复、工具摘要和等待窗口缓存转正结果。内部持久历史可按执行顺序保存交错的工具调用记录、内部工具名和已脱敏参数占位。图片工具摘要只包含操作类型、成功或失败状态和安全消息，不包含后端显示名。

后续格式化为模型上下文时会过滤这些工具协议块，这些工具协议块不会发送到群聊。Provider ID、凭据、API 地址、原始响应和文件路径不会进入安全摘要或群聊文本。
